...
import { Filter, X, ChevronDown, ChevronUp, Search as SearchIcon, Star, ShoppingCart, Tag, TrendingUp, Sun, CheckCircle, SlidersHorizontal, CalendarIcon } from 'lucide-react';
...
// Backend API endpoint for browsing inventory products
const API_URL = apiUrl('/api/inventory/');
const CART_ITEMS_URL = apiUrl('/api/cart/items/');

const categories = [
  { name: 'Machinery', icon: <Tag className="w-5 h-5" /> },
  { name: 'Farming Tool', icon: <Tag className="w-5 h-5" /> },
  { name: 'Vegetables', icon: <Sun className="w-5 h-5" /> },
  { name: 'Fruits', icon: <Tag className="w-5 h-5" /> },
  { name: 'Seeds', icon: <Tag className="w-5 h-5" /> },
  { name: 'Fertilizer', icon: <Tag className="w-5 h-5" /> },
  { name: 'Pesticide', icon: <Tag className="w-5 h-5" /> },
  ...
  { name: 'Other', icon: <Tag className="w-5 h-5" /> },
];
...
function BadgeWithTooltip({ badge }) {
  const badgeInfo = {
    ...
    'Newly Added': 'This product was recently added to our inventory.',
    'Featured': 'This product is specially featured by our team.',
  };
    ...
    >
      <div className="relative">
        ...
        <div className="absolute top-3 right-3 z-10">
          <button
            ...
            disabled={!isCustomer || product.stock === 0 || adding}
            onClick={async e => {
              e.stopPropagation();
              if (!isCustomer) {
              ...
              try {
                const res = await fetch(CART_ITEMS_URL, {
                  ...
                  body: JSON.stringify({ inventory_product_id: product.id, quantity: 1 })
                });
                if (!res.ok) {
      ...
      <div className="flex-1 flex flex-col px-4 pt-3 pb-4">
        <div className="flex items-center gap-2 mb-1 flex-wrap">
          ...
          {product.badge && <BadgeWithTooltip badge={product.badge} />}
        </div>
        ...
        <div className="flex items-center gap-2 mt-auto">
          ...
          {product.stock === 0 && <span className="ml-2 text-xs text-red-500 dark:text-red-400 font-semibold">Out of Stock</span>}
        </div>
      </div>
    </div>
  );
...
export default function OurInventory() {
  const { userType, isAuthenticated, profile } = useAuth();
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        if (filters.inStock) params.push('stock__gt=0');
    ...
    <div className="relative min-h-screen bg-gradient-to-br from-emerald-50 to-green-100 dark:from-gray-900 dark:to-gray-800 font-sans text-gray-800 dark:text-gray-200">
      ...
      <div className="max-w-7xl mx-auto flex gap-8 pt-24 pb-12 px-4 sm:px-6 lg:px-8">
        ...
        <main className="flex-1">
          <FadeInOnView>
            <h1 className="text-3xl md:text-4xl font-bold mb-6 text-emerald-800 dark:text-emerald-400 drop-shadow">Our Inventory</h1>
          </FadeInOnView>
          <div className="mb-6 flex justify-center">
            <div className="relative w-full max-w-4xl">
              <input
                ...
                placeholder="Search for inventory, e.g. Tools, Seeds, Machinery..."
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6">
            {loading ? (
              ...
              <div className="col-span-full text-center text-gray-500 dark:text-gray-400 py-16 animate-fade-in-up">
                ...
                <div className="text-lg font-semibold mb-4">No inventory found for your filters.</div>
